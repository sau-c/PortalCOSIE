...
namespace PortalCOSIE.Application.Features.Usuarios.Commands.ToggleAlumnoRol
{
    public class ToggleAlumnoRolHandler : IRequestHandler<ToggleAlumnoRolCommand, Result<string>>
    {
        ...
        public async Task<Result<string>> Handle(ToggleAlumnoRolCommand command)
        {
            ...
            if (command.CertificadoCer == null)
                return Result<string>.Failure("Debes cargar el certificado .cer del alumno para activarlo.");

            var asignar = await _certificadoAlumnoService.AsignarCertificadoAsync(command.UserId, command.CertificadoCer);
            if (!asignar.Succeeded)
                return asignar;
